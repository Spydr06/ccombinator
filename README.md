...
Functions returnin `int` like `cc_parse` return `0` on success or a non-zero errno value on failure.
...
The following functoins aid constructing a parser by supplying a regular expression:
```c
